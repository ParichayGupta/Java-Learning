...
class SearchDemo extends JPanel implements ActionListener
{
	...
	SearchDemo()
	{
		...
		u2=new JLabel("info of entered RNO. is...");
		u2.setSize(120,30);
	...
	public void actionPerformed(ActionEvent e1)
	{
		...
		try
		{
			...
			String db_url="jdbc:mysql://localhost:3306/avani?useSSL=false";
			String db_name="root";
			String db_upass="root";

			Connection con=DriverManager.getConnection(db_url,db_name,db_upass);
			Statement st=con.createStatement();
		
			String q= "select * from insmarks where RNO='"+s1+"'";
			
			ResultSet rs=st.executeQuery(q);
			
			if(rs.next())
			{
				u2.setText(rs.getString(2));
				u3.setText(rs.getString(3));
				
			}
			else
			{
				JOptionPane.showMessageDialog(b1,"Invalid RNO...");
			}
			t1.setText("");
			con.close();
			}
